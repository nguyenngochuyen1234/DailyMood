...
import { 
  ...
  ScrollView, Image, Dimensions, ImageBackground, Modal
} from 'react-native';
import { 
...
export default function FolderScreen({ navigation }: { navigation: any }) {
  ...
  React.useEffect(() => {
    const unsubscribe = navigation.addListener('focus', async () => {
      ...
      allJournals.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()).forEach(j => {
        if (j.journeyId) {
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <View style={styles.header}>
            <Text style={[styles.headerTitle, { color: colors.text.dark }]}>Hành trình</Text>
          </View>

          <ImageBackground
            source={backgrounds.add}
            style={styles.banner}
            imageStyle={{ borderRadius: SIZES.radius.xxl }}
          >
            <View style={[styles.bannerOverlay, { backgroundColor: 'rgba(255,255,255,0.4)' }]}>
          ...
          {journeys.map((item: any) => (
            <TouchableOpacity
              key={item.id}
              style={[styles.journeyCard, { backgroundColor: colors.backgroundCard }]}
              onPress={() => navigation.navigate("JourneyDetail", { journey: item })}
            >
              <View style={styles.cardHeader}>
                <View style={[styles.badge, { backgroundColor: `${colors.primary}20` }]}>
                  <Text style={[styles.badgeText, { color: colors.primary }]}>{item.postCount} BÀI VIẾT</Text>
                </View>
              ...
              <View style={styles.cardMain}>
                <View style={styles.textColumn}>
                  <Text style={[styles.journeyTitle, { color: colors.text.dark }]}>{item.name}</Text>
                  <Text style={[styles.journeyDesc, { color: colors.text.dark }]} numberOfLines={2}>{item.description || "Hãy tiếp tục viết nên hành trình tuyệt vời của bạn."}</Text>
                  <View style={styles.ctaButton}>
                    <Text style={[styles.ctaText, { color: colors.primary }]}>Tiếp tục viết</Text>
                ...
                {item.lastImage && (
                  <Image source={{ uri: item.lastImage }} style={[styles.lastPhoto, { borderColor: colors.border }]} />
                )}
              </View>
            </TouchableOpacity>
          ))}
          ...
          <TouchableOpacity 
            style={[styles.bottomActionButton, { backgroundColor: colors.primary }]}
            onPress={() => setShowCreateModal(true)}
          >
             {React.createElement(Compass as any, { size: 20, color: colors.text.textOnDark, style: { marginRight: 8 } })}
             <Text style={[styles.bottomActionText, { color: colors.text.textOnDark }]}>Bắt đầu Hành trình mới</Text>
          </TouchableOpacity>
