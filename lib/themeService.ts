...
export const fetchThemesFromSupabase = async (): Promise<ParsedTheme[]> => {
  try {
    ...
    if (bgError) {
      console.warn(
        "Could not fetch backgrounds, using default colors only:",
        bgError,
      );
    }
    ...
    const parsedThemes: ParsedTheme[] = themesData.map((theme: any) => {
      ...
      const relatedBackgrounds = bgData
        ? bgData.filter((bg: any) => bg.theme_id === theme.id)
        : [];
      ...
      const parsedColors: ThemeColors = {
        ...
        background: {
          ...COLORS.background,
          tabBar:
            typeof colorsJson.tabBar === "string"
              ? colorsJson.tabBar
              : COLORS.background.tabBar,
          main:
            typeof colorsJson.background === "string"
              ? colorsJson.background
              : COLORS.background.main,
          ...(typeof colorsJson.background === "object"
            ? colorsJson.background
            : {}),
        },
        text: {
          ...
          dark:
            colorsJson.text && colorsJson.text.dark
              ? colorsJson.text
              : colorsJson.text || COLORS.text.dark,
          muted:
            colorsJson.text && colorsJson.muted
              ? colorsJson.muted
              : colorsJson.muted || COLORS.text.muted,
          textOnDark:
            colorsJson.text && colorsJson.textOnDark
